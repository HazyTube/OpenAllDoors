...
namespace OpenAllDoors
{
    internal static class Updater
    {
        ...
        public static int CheckUpdate()
        {
            ...
            try
            {
                Game.LogTrivial("Fetching latest plugin version from GitHub");
                response = wc
                    .DownloadStringTaskAsync(new Uri(
                        "https://raw.githubusercontent.com/HazyTube/OpenAllDoors/master/PluginVersionInfo/LatestVersion"))
                    .Result;
            }
            catch (Exception)
            {
                Game.LogTrivial($"Checking version of plugin {Globals.Application.PluginName} Failed");
            }
